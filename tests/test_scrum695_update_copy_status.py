...
class FakeCopyDocumentReference:
    ...
    def get(self):
        copy_data = self.database.copies.get(self.book_id, {}).get(self.copy_id)
    ...
    def update(self, updated_data):
        if self.copy_id not in self.database.copies.get(
            ...
            raise KeyError(f"Unknown copy ID: {self.copy_id}")

        self.database.copies[self.book_id][self.copy_id].update(dict(updated_data))
...
def test_scrum_695_update_status_page_loads(
    ...
    response = client.get(("/books/copies/status/" "BOOK001/COPY-BOOK001-001"))
...
def test_scrum_695_accepts_each_valid_status(
    ...
    updated_copy = fake_database.copies["BOOK001"]["COPY-BOOK001-001"]
...
def test_scrum_695_updates_only_selected_copy(
    ...
    second_copy_before = dict(fake_database.copies["BOOK001"]["COPY-BOOK001-002"])
    ...
    first_copy = fake_database.copies["BOOK001"]["COPY-BOOK001-001"]

    second_copy = fake_database.copies["BOOK001"]["COPY-BOOK001-002"]
...
def test_scrum_695_records_one_update_operation(
    ...
    assert len(fake_database.copy_update_history) == 1

    update_record = fake_database.copy_update_history[0]

    assert update_record["book_id"] == "BOOK001"

    assert update_record["copy_id"] == ("COPY-BOOK001-001")

    assert update_record["updated_data"]["status"] == "Reserved"
...
def test_scrum_695_displays_success_message(
    ...
    assert (
        ...
        b"to Damaged successfully." in response.data
    )
...
def test_scrum_695_rejects_invalid_status(
    ...
    original_status = fake_database.copies["BOOK001"]["COPY-BOOK001-001"]["status"]
    ...
    assert b"Please select a valid copy status." in response.data

    current_status = fake_database.copies["BOOK001"]["COPY-BOOK001-001"]["status"]

    assert current_status == original_status

    assert fake_database.copy_update_history == []
...
def test_scrum_695_unknown_book_returns_404(
    ...
    response = client.get(("/books/copies/status/" "UNKNOWN/COPY-BOOK001-001"))
...
def test_scrum_695_unknown_copy_returns_404(
    ...
    response = client.get(("/books/copies/status/" "BOOK001/UNKNOWN-COPY"))

    assert response.status_code == 404

    assert b"Physical book copy not found." in response.data
...
def test_scrum_695_summary_changes_after_status_update(
    ...
    details_response = client.get("/books/details/BOOK001")
